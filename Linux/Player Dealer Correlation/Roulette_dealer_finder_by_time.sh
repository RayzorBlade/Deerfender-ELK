...
validate-args()
{
  ...
  # Validate that the second suppied argument meets the required format. Eg. 03AM, 12PM etc.
...
if [ "$result" != "" ]; then
  ...
  if [ ! -f "$dsfile" ]; then
    ...
    echo ""
    echo $(echo "$line" | awk -F'\t' '{printf "%s was the Roulette dealer working at %s on this day", $3, $1 }')
    echo ""
